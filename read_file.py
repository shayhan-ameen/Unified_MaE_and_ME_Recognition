...
def read_csv(path: str, num_classes) -> tuple:
    ...
    if num_classes == "Folder":
        data.loc[data["Filename"].str.contains("anger"), "Estimated Emotion Folder"] = (
            "anger"
        )
        data.loc[
            data["Filename"].str.contains("disgust"), "Estimated Emotion Folder"
        ] = "disgust"
        data.loc[data["Filename"].str.contains("happy"), "Estimated Emotion Folder"] = (
            "happy"
        )
    ...
    label_mapping = {
        emotion: idx
        for idx, emotion in enumerate(np.unique(data.loc[:, emotion_categories]))
    }
    print(label_mapping)
